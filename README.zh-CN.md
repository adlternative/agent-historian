...
- **零运行时依赖。** 无原生模块。支持 **Node ≥ 18**；其中 OpenCode 源额外需要 **Node ≥ 22.5**（内置 `node:sqlite`），在更老的 Node 上会自动降级为"不可用"，其余源照常工作。
...
需要 **Node ≥ 18**。其中 OpenCode 源需要 **Node ≥ 22.5**（内置 `node:sqlite`）；在更老的 Node 上它会显示为不可用，其余源仍可正常使用。
